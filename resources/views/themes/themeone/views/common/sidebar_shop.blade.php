@if($result['filters'])
 ...
 <div id="accordion"  class="filters" role="tablist">
 	...
 	<form enctype="multipart/form-data" name="filters" method="get">
        ...
        <input type="hidden" name="max_emp" id="max_emp" value="60">
        <input type="hidden" name="min_age" id="min_age" value="0">
        <input type="hidden" name="max_age" id="max_age" value="60">
        <input type="hidden" name="min_payback" id="min_payback" value="0">
        <input type="hidden" name="max_payback" id="max_payback" value="60">
        ...
        <div class="card">
          ...
          if(!empty($result['filter_attribute']['option_values']) and in_array($values['value_id'],$result['filter_attribute']['option_values'])) print 'checked';
			...
			?>
        	<a href="{{ URL::to('/shop'.$url)}}" class="btn btn-dark" id="apply_options" style=" background-color: green"> @lang('website.Reset') </a>
             @if(app('request')->input('filters_applied')==1)
        	<button type="button" class="btn btn-secondary" id="apply_options_btn" style=" background-color: red"> @lang('website.Apply')</button>
            @else
        	<!--<button type="button" class="btn btn-secondary" id="apply_options_btn" disabled> @lang('website.Apply')</button>-->
            <button type="button" class="btn btn-secondary" id="apply_options_btn" style=" background-color: red; color: white"> @lang('website.Apply')</button>
            @endif
        </div>
		</div>
  </form>
